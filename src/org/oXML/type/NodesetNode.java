...
public class NodesetNode extends SimpleNode  {
    ...
    */
	...
	if(child.getType().instanceOf(NodesetNode.TYPE)){
	    Nodeset set = child.getChildNodes();
	    for(int i=0; i<set.size(); ++i)
		kids.addNode(set.getNode(i));
	}else{
	    kids.addNode(child);
	}
    }
	...
	if(child.getType().instanceOf(NodesetNode.TYPE)){
	    Nodeset set = child.getChildNodes();
	    for(int i=0; i<set.size(); ++i)
		kids.insertNode(pos++, set.getNode(i));
	}else{
	    kids.insertNode(pos, child);
	}
    }
	...
	if(child.getType().instanceOf(NodesetNode.TYPE)){
	    Nodeset set = child.getChildNodes();
	    for(int i=0; i<set.size(); ++i)
		removeChild(set.getNode(i));
	}else{
	    int pos = kids.indexOf(child);
	    if(pos < 0)
